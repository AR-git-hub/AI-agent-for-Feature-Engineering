...
def ensure_env_file() -> None:
    ...
    assert any(env_var.startswith("GIGACHAT_CREDENTIALS") for env_var in content), (
        "В .env отсутствует GIGACHAT_CREDENTIALS"
    )
...
def ensure_dependencies() -> None:
    ...
    required_markers = ["catboost", "pandas", "numpy", "langchain-gigachat", "python-dotenv"]
...
def main() -> None:
    ...
    try:
        ...
        raise AssertionError(f"Решение превысило лимит времени {MAX_RUNTIME_SEC} секунд") from e
    ...
    assert elapsed <= MAX_RUNTIME_SEC, (
        f"Решение работало слишком долго: {elapsed:.2f} сек. " f"Лимит: {MAX_RUNTIME_SEC} сек."
    )
